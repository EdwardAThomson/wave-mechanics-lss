...
Whether these equations can be applied to the study of quantum nature of gravity is less clear. A yet-unsolved problem in quantum physics is the cause of wavefunction collapse: ``why one version of reality over another?''. Penrose \citep{bib_pen} has suggested a way that gravity might cause the collapse of a wavefunction (Penrose Interpretation of quantum mechanics). He used the Schr\"odinger-Newton (same as: Schr\"odinger-Poisson) equations to describe the basis states of his theory. This is noteworthy because these are the same equations used in this thesis, albeit applied here to a very different physical regime. This theory incorporates ideas of quantum mechanics and gravity but does not appear to be a typical theory of quantum gravity. The latter is based upon the notion that space, as well as time, is quantized into discrete amounts.
...
The focus of this thesis is to investigate a simulation technique based in wave-mechanics that reproduces the observed patterns of Large Scale Structure. The key concepts of the Standard Model (listed above) feed into the generation of initial conditions and the evolution of the background space-time model; these ideas and parameters instruct us on how to write a code that faithfully reproduces the patterns of LSS.  It may not be obvious that the Schr\"odinger formalism can be applied to Large Scale Structure, so part of our work was to perform numerical experiments that investigate how the Schr\"odinger equation behaves.\footnote{These preliminary experiments included reflection and tunnelling scenarios. While they were valuable for building intuition and verifying the numerical implementation, they are not directly relevant to the LSS results and are omitted from this thesis.} Consequently, the LSS simulations in this thesis were constructed in a way that is consistent with this Standard Model (see Chapter \ref{ch_5}). Some ideas for using wave-mechanics to probe beyond the Standard Model are presented as speculative ideas in Chapter \ref{ch_7}.
...
\paragraph{Friedmann equation.} The above metric for space-time is often called the Robertson-Walker (R-W) metric, named after the two physicists that discovered it independently of Friedmann. However, it was from this metric that Friedmann went on to find his famous solution to the field equations \ref{eqn_efe} and hence derive equations of motion for the Universe. In addition to using the RW metric, he also assumed the energy-momentum tensor of a perfect fluid which ensures energy and momentum conservation via the covariant divergence condition $T_{\mu\nu;\nu} = 0$ \citep{bib_schutz}. His equation is given as:
...
The principle of general covariance ensures that General Relativity has no preferred observers: the laws of physics are the same everywhere. Combined with the assumed symmetries (the R-W metric and the symmetric energy-momentum tensor $T_{\mu\nu}$), Slipher's and Hubble's observations lead to the conclusion that all points --- and observationally, all galaxies --- are mutually receding from each other. The Universe appears to be expanding.

However, it is worth pausing to consider what ``expansion'' actually means. General Relativity describes the geometry of space-time but does not endow empty space with substance; there is nothing to stretch. The scale factor $a(t)$ in the Friedmann equation is a property of the chosen coordinate system, not a statement about space being a medium that physically expands. The cosmological constant similarly enters as a geometric term in the field equations and does not provide a mechanism that induces expansion.

What Hubble observed was redshifted electromagnetic radiation, not a direct observation of space itself expanding. Bunn and Hogg \citep{bib_bunn} have shown that cosmological redshift can be understood entirely as the accumulation of infinitesimal Doppler shifts along the photon's path through curved space-time, without invoking ``expanding space'' as a causal mechanism. Peacock \citep{bib_pea2} reaches a similar conclusion: at the level of the equations of motion, galaxies move through space under gravity rather than being carried apart by an expanding substrate.

This distinction is not merely philosophical. In the cosmological simulations of this thesis, the equations of motion are written in comoving coordinates where the scale factor governs the background evolution, but the density field evolves \emph{through} that coordinate system. The comoving frame is a coordinate choice, not a claim that space is a substance being stretched. If a future theory of quantum gravity reveals that the vacuum has non-trivial structure, this conclusion may need to be revisited.
...
The concept of temperature becomes problematic in this regime, as it requires a system in or near thermal equilibrium. In such a highly relativistic and quantum-dominated regime as the earliest moments after the Big Bang, equilibrium cannot be assumed and talking of temperature is misleading. However, it is possible to calculate the energy of elementary particles at this time (as order of magnitude estimates) and hence establish a pseudo-temperature. From the Cosmological Principle it is possible to extrapolate that the Universe by necessity should be isotropic and homogeneous at this time too. By implication then the Universe will have a near constant energy density.
...
That is to say that the Universe would be in causal contact before inflation and because of the superluminal expansion the Universe will still appear to be in thermal equilibrium even though the actual horizon size is much smaller than the size of the Universe. This is why the Universe can appear to be in thermal equilibrium at the time of Last Scattering.
...
\noindent here power, in Fourier space $P_k$, follows a power law in wavenumber $k$. The parameter $A$ sets the overall normalization of each mode. The power spectrum tells us about the correlations in the density (here, over-density $\delta$) field.
...
There are two main contributions to primordial density perturbations: an adiabatic component and an isocurvature component. In general, the perturbations will be a mixture of both. The abundance of each type is a feature of power spectrum of the CMB, therefore it is clearly possible to say something about the nature of inflation given the CMB power spectrum (see below).
...
It is expected that hot and cold spots should thermalize and hence appear as a uniform temperature (that of Hydrogen ionization); however, denser spots actually recombine later and so they are less red-shifted and therefore hotter. As a rough heuristic, we can relate the fractional density difference $\delta \rho / \rho$ to the fractional temperature difference $\delta T / T$:

\begin{equation}
\frac{\delta T}{T} \approx - \frac{\delta z}{1+ z} \approx \frac{\delta \rho}{\rho}
\end{equation}

\noindent here we assume that the growth of density perturbations is linear, $\delta \propto (1 +z)^{-1}$, and neglect the full Sachs-Wolfe and acoustic contributions. To linear order the fluctuations in temperature are of the same order as the fluctuations in density. Therefore the size of density perturbations at this time will also be about $10^{-5}$.
...
\paragraph{Dark Energy.} The last piece of the puzzle that needs to be mentioned is that of the ambiguously named Dark Energy, which bears no resemblance to Dark Matter. The two are not equivalent or interchangeable as one might naively guess from Einstein's relation of mass to energy. Unfortunately, the name has stuck despite the misleading nature of its meaning.
...
As the supernovae have a known absolute brightness, their luminosity distance can be compared with the distance inferred from their redshift. \citet{bib_riess} showed that the high-redshift supernovae are dimmer --- and therefore farther away --- than predicted by a decelerating, matter-dominated Universe. This implies that the expansion of the Universe is accelerating.
...
It is possible to directly code these equations on a computer and generate simple results for LSS. Using the Boltzmann equation is only one such approach, as already stated the main technique is the $N$-body method.

In summary, this chapter has introduced the key components of the $\Lambda$-CDM model that are relevant to simulations of Large Scale Structure: the Friedmann equation governing cosmic expansion, the observational constraints from the CMB and supernovae, and the fluid description of collisionless matter that underpins both the $N$-body and wave-mechanical approaches. The next chapter (\ref{ch_2}) reviews the standard simulation techniques in LSS, with particular focus on the $N$-body method and where it can be improved upon.
