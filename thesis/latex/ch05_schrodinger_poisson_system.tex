...
\label{split}
The extension to three dimensions requires combining three separate ideas, each addressing a different aspect of the problem:

\paragraph{1. Operator splitting (Strang/Suzuki):} The kinetic and potential energy operators do not commute, so the full evolution $e^{-i(K+V)dt}$ cannot be naively factored. The standard symmetric (Strang) splitting provides a second-order accurate decomposition:

\begin{equation}
\psi(x, t+dt) = e^{-i (K+V) dt} \psi(x, t) \approx e^{-i K dt/2}\, e^{-i V dt}\, e^{-i K dt/2} \psi(x, t)
\end{equation}

\noindent This is the same splitting discussed in Chapter \ref{ch_3}. It separates the evolution into a kinetic half-step, a full potential step, and a second kinetic half-step. Watanabe \citep{bib_wata} and Suzuki \citep{bib_suzu} have shown how to construct higher-order decompositions; however, the second-order form is sufficient for our purposes.

\paragraph{2. Dimensional splitting:} The momentum operators in different directions commute: $[P_x, P_y] = 0$. This means the kinetic energy can be exactly factored across dimensions without approximation:
...
\noindent where the second line shows each 1D kinetic step written in Cayley form. Each factor is a tridiagonal system solved by the Goldberg method (section \ref{onedim}).

\paragraph{3. The Goldberg solver:} Each of the 1D tridiagonal solves above uses the Crank-Nicolson scheme from section \ref{onedim}. The Goldberg method is the workhorse that performs each individual sweep; operator splitting and dimensional splitting determine how these sweeps are assembled into a full 3D timestep.
...
\noindent Here $\lambda = 4\epsilon^2/\delta$. The form for $f$ remains unchanged while the $\Omega$ now has no $V$ term. Updating the wavefunction for the potential is straightforward:
...
As point of comparison, we will provide an alternative set of plots (figure \ref{fig.tophat2}) that use a larger value of $\nu$ ($10^{-7}$). This alternative set of plots shows that the system evolves more rapidly: the larger $\nu$ drives faster dispersion, and the over-density reaches the same peak ($\delta = 8.33$) by timestep 800. However, unlike the $\nu = 10^{-8}$ case, the peak is not sustained — it soon drops in height and broadens as dispersion dominates over gravitational attraction. The over-density remains centralised but gravity is not strong enough to hold the region as a quasi-singular point against the larger dispersive pressure. In tests where $\nu$ is much larger the over-density expands so fast that the wavefunction crosses over the boundaries. This provides feedback into the system where different frequencies within the wavefunction can cause an interference pattern. The result is an unsmooth wavefunction with rapid variability.
...
It is important to distinguish between two classes of discrepancy with GADGET-2. The first is resolution-dependent: the choice of grid size and $\nu$ controls how finely the wavefunction can represent structure, and the rapid gridpoint-to-gridpoint variability seen in figure \ref{fig.wmrho} may diminish at higher resolution. The second is potentially intrinsic to the wave-mechanical framework: a possible source of such messiness is due to the fact that the whole simulation uses a single coherent wavefunction. As shown in the results of section \ref{per_bou}, a single wavefunction can lead to interference effects. Although we hope to limit this effect by using a small $\nu$ we may not have completely suppressed these effects. That is despite the fact gravity also acts to suppress such interference effects.
