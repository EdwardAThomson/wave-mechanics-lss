...
In the thirteen years since Bertschinger's review paper further progress has been made and one of the first billion-body ($N = 1024^3$) gravitational simulations (Millennium Simulation \citep{bib_spr}) was carried out in the year 2005. The code used (GADGET-2) in this simulation combines a Tree algorithm for short-range gravitational forces with a Particle-Mesh (PM) method for long-range forces (a TreePM scheme), and also includes Smoothed Particle Hydrodynamics (SPH) for gas dynamics (omitted in this thesis for brevity). Both P-P and P-M methods are outlined below in sections \ref{sec.direct} and \ref{sec.mesh}.
...
\citet{bib_he} note that this $\epsilon$ parameter was soon introduced into simulations of star systems, by necessity to keep the simulation numerically stable, even though a point-mass simulation ($\epsilon = 0$) is a better physical model. These authors point out that it is not good computational practice to allow any variables to reach exceptionally large values as time integration becomes inaccurate and arithmetic overflow may occur.
...
\noindent Here $\epsilon \ll 1$ is the softening parameter (a small positive length). The softening parameter is phenomenological: it does not appear naturally in the equations but is a computational fix. This addition of a softening parameter is a phenomenological way of preventing artificial two-body relaxation. However, it is clear that without this parameter then two such particles can dominate the entire energy distribution of the system.
...
\paragraph{Integrator type.} Typically, the equations of motion are solved using a `Leapfrog' integrator. This is a second-order accurate method (global error $O(h^2)$) and, crucially, it is symplectic: it preserves the phase-space structure of the Hamiltonian system and keeps energy errors bounded (see \ref{sec.sym} for a definition of symplectic). An alternative scheme would be to use the fourth-order Runge-Kutta integrator (RK4) which has a smaller global error of $O(h^4)$; however, it is not symplectic and so exhibits secular energy drift over long integrations. \citep{bib_nr}
...
Direct summation methods have been used to study many astrophysical systems and still prove to be popular. They have been used to study solar system dynamics, planet formation, galaxy formation and galaxy clustering \citep{bib_quin, bib_aar}. Modern Cosmology codes prefer a mix of direct summation (P-P) and the particle-mesh (P-M) method \citep{bib_bert, bib_hmpc}, which are colloquially called $P^3 M$ (``$P$-cubed-$M$").
...
\noindent which can be read as the curvature of the gravitational potential $\Phi$ being sourced by the density perturbation $\bar\rho\delta$, where $\delta = (\rho - \bar\rho)/\bar\rho$ is the overdensity. The above version of the equation is in comoving cosmological form, where the scale factor $a$ accounts for expansion. This eventually leads to the calculation of force (or acceleration) that appears in the equations of motion for a cosmological $N$-body code.
...
\label{sec.sym}
Symplectic numerical integrators are designed specifically for Hamiltonian systems. They preserve the geometric structure of Hamilton's equations --- the symplectic structure --- which in turn ensures that the qualitative long-term behaviour of the system is faithfully reproduced. See \citep{bib_saha} and references therein.
...
\noindent Here the exterior wedge product of the coordinate differentials defines a phase-space area element. The non-degeneracy condition means that $\omega$ has no null directions: for any non-zero vector in phase space, there exists another vector with which it has a non-zero symplectic product. This ensures that every degree of freedom in the system is fully captured by the symplectic structure.

The condition of being closed ($\mathrm{d}\omega = 0$) means that the symplectic form is preserved under smooth deformations; it is a topological condition ensuring that the phase-space structure does not change locally. The requirement of differentiability means that $\omega$ is smooth everywhere on the manifold. \citep{bib_tens}

In simple terms, the Hamiltonian (energy function) of a system is a smooth real-valued function on a symplectic manifold (commonly referred to as phase space). Any transformation that preserves the symplectic structure is called a symplectic transformation, or symplectomorphism. Hamiltonian time evolution is one such transformation. Liouville's theorem shows that symplectomorphisms preserve volume in phase space ($\mathrm{d}q \wedge \mathrm{d}p$). It is important to note that phase-space volume conservation does not by itself imply energy conservation. Rather, a symplectic integrator conserves a nearby `shadow' Hamiltonian that differs from the true Hamiltonian by a small bounded amount. The energy error therefore oscillates but does not grow secularly, which is what makes symplectic integrators superior for long-time integrations.

A key property of Hamiltonian flow is that it provides a continuous family of symplectomorphisms: any state in phase space can be smoothly evolved forward or backward in time. This continuity is closely related to the variational principle, where the physical trajectory is determined by extremising the action and the endpoints are fixed while the path between them can be continuously deformed.

From Noether's theorem we know that behind every continuous symmetry is a conservation law \citep{bib_noe}. The symplectic structure of Hamiltonian mechanics encodes these symmetries geometrically: time-translation symmetry gives energy conservation, spatial symmetry gives momentum conservation, and so on. The smoothness and differentiability of the symplectic manifold are what make these conservation laws possible. A numerical integrator that preserves this symplectic structure therefore respects the underlying symmetry--conservation relationship, even if individual conservation laws (such as exact energy conservation) hold only approximately. \citep{bib_duff}

Therefore we are inclined to look for a numerical integrator that is symplectic. While such an integrator does not conserve energy exactly, it keeps the energy error bounded for all time, which is far preferable to the secular energy drift seen in non-symplectic schemes. This is an important property for cosmological codes as the exact positions of particles are less relevant due to the statistical nature of the system. One specific realisation of particle positions is no more important than another, provided they are statistically equivalent (for example, the same degree of clustering: equal values of $\sigma_{8,0}$).
...
The first line is the continuity equation for the density perturbation $\delta$, the second is the linearised Euler equation (the non-linear advection term $v \cdot \nabla v$ has been dropped), and the third is the Poisson equation for gravity. The simplest models of perturbation theory often have an analytic solution. One example is the evolution of a CDM-fluid that only includes linear terms in the dynamics: the solution is the well known Einstein de Sitter Universe where $ a \propto t^{2/3}$ and corresponding solutions for $\delta \propto t^n$ are $n = 2/3$ (growing mode) or $-1$ (decaying mode).
...
Fluid method: Peebles \citep{bib_peeb2} used a pressureless fluid ($\nabla . v = 0$) in an expanding Universe. He evolved the coupled continuity, Euler, and Poisson equations for the density and velocity fields (see equation \ref{eqn_linflu}), ensuring mass and momentum conservation. Matter is treated as a continuous field; however, velocity dispersion must be negligible.
\newline
