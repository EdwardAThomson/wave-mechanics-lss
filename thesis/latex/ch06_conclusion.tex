...
\chapter{Conclusion}
\label{ch_6}
This thesis provides an overview of wave-mechanics as applied to LSS. The interpretation of the Schr\"odinger equation was re-examined in Chapter \ref{ch_3}, previous inconsistencies and misconceptions were explored and explored and, where possible, resolved. Our understanding of wave-mechanics as applied to LSS should now be clearer and more concise. The approach in this thesis is purely classical due to the high particle occupation number in each quantum state, so pure quantum mechanics is never present. Whether dark matter particles exhibit quantum behaviour at astrophysical scales remains an open question (cf.\ the ultralight boson hypothesis discussed in Chapter \ref{ch_3}); however, this thesis treats the Schr\"odinger equation as a practical modelling tool in the classical limit, not as a statement about the quantum nature of dark matter. This distinction does not affect the main outcomes of the work.
...
The 1D and 3D toy models considered in Chapter \ref{ch_4} were a demonstration of a consistent framework. I have shown that the FPA can handle cosmological initial conditions by providing results that are comparable to the widely available $N$-body codes (at least in the quasi-linear regime). The benefit of the FPA is that, in the quasi-linear regime, it requires only a single FFT step regardless of the target time, making it extremely fast compared to time-stepping methods. It merely requires a single step of computation due to the symmetry of the equations.

After developing and testing the FPA, I went on to investigate another method of solving the Schr\"odinger equation, commonly called the Cayley method (Chapter \ref{ch_5}) as it involves describing the exponentials via Cayley's approximation. In this approach we no longer assume the trick that was inherent to the FPA, where the effective potential is zero. In the Cayley approach we solved the full system of equations: both the kinetic term and solved Poisson's equation of gravity.

As shown in Chapter \ref{ch_5}, this process is trickier than it may first appear. Under the recommendation of Watanabe's publication we adopted Suzuki splitting operators and also adopted Widrow \& Kaiser's method of re-writing the equations into a simplified form that deals with timesteps as equal steps in $\ln a$. Fortunately, each component works consistently with one another. The simulations prove to be stable and conserve key quantities, though significant issues remain with the velocity field (see below). Subject to these caveats, the main goals of developing a wave-mechanics code for LSS were achieved. Those goals were: (1) 3D coordinates, (2) self-consistent gravity, (3) expanding coordinates, (4) periodic boundaries, (5) mass conserving (and, importantly, it also conserves momentum --- a strong validation of the symplectic integration scheme). In addition this also satisfies the original goals suggested by Widrow \& Kaiser: A model that describes collisionless matter; matter is described as a field, not particles: that is, continuous; function only of space and time (3+1)-dimensions; follow multiple streams in phase space (`hot' / dispersive); competitive with $N$-body techniques for computing time. Sabiu's run with GADGET-2 took approximately the same time to complete as my Wave-mechanics code, of the order of a few hours; however, no rigorous speed testing was conducted in this thesis. Both codes were run on different machines so does not represent a fair comparison but I do expect both codes to be a comparable speed. The number of operations required in each case are similar in magnitude.
...
Throughout this work I have highlighted the similarities between wave-mechanics and $N$-body simulations. The latter was used as a benchmark for speed and accuracy. However, I have shown wave-mechanics provides a different method for simulating Large Scale Structure. It has many benefits but it lacks the maturity and development seen in modern $N$-body codes. In principle, a wave-mechanics code that has been developed further and optimised for high-performance machines could become competitive with $N$-body codes in speed; however, this has not yet been demonstrated and would require rigorous benchmarking. In terms of density resolution, the continuous density field representation should reduce shot-noise artefacts that arise from finite particle numbers in $N$-body codes, though wave-mechanics still uses a grid and is therefore subject to its own resolution limits. Note that this advantage pertains specifically to the density field; velocity fields remain an open problem as discussed below. The allowance of multi-streaming also prevents two-body relaxation that is a critical issue for $N$-body codes, the current solution to that problem is unphysical and is completely avoided in wave-mechanics.

\subsection*{Limitations}
Two major unresolved issues must be acknowledged. First, the velocity distributions from the wave-mechanics code are systematically wider than those from GADGET-2, and a spurious bulk flow develops over the course of the simulation (see Chapter \ref{ch_5}, section \ref{known_issues}). These velocity problems may originate from the construction of the initial wavefunction using the gravitational potential rather than the velocity potential, or from an as-yet-unidentified implementation error. Second, no systematic convergence study was performed: while higher-resolution runs ($128^3$) appeared better behaved, this was not quantified. A rigorous resolution study varying both grid size and $\nu$ is essential before the code can be used for quantitative predictions.

The main problems with the results presented in this thesis were the unexpected, although not necessarily inconsistent, results for density and velocity. The messiness of the evolved density fields is potentially due to interference arising from the use of a single coherent wavefunction for the entire simulation volume. This is an intrinsic feature of wave-mechanics that has no counterpart in $N$-body codes, and is not something one would desire when modelling a classical system. This effect requires further investigation.

The simulations presented were run at a coarse spatial resolution of $64^3$, this was a problem for generating our initial conditions and hence our subsequent results. At higher resolutions the results appeared better behaved, though this observation is suggestive rather than conclusive in the absence of a systematic convergence study. The higher-resolution runs were prohibitively more expensive to run within the time frame of this thesis. In order to make sense of the lower resolution simulations we smoothed the data using a Gaussian filter (essentially a low pass filter that takes out high frequency `noise'). This operation gave data that looked closer to the GADGET-2 data. We would have liked to use the raw, unsmoothed data, for our point of comparison but it did not look consistent with what we first expected.
...
\begin{itemize}
\item Multiple fluids (engineering: straightforward extension of existing framework)
\item The splitting operator approach to including more physics, such as pressure (mathematically non-trivial: operator decomposition must preserve unitarity)
\item Periodic boundaries via adhesive operators (engineering: requires careful implementation but theory exists)
\item Mesh refinement (mathematically difficult: Poisson solver on non-uniform grids is a major obstacle)
\item Parallelization (mathematically difficult: recursion relations are inherently sequential)
\item Including vorticity and spin (speculative: extends the formalism from the Schr\"odinger to the Pauli equation, not yet attempted in an LSS context)
\end{itemize}
...
Johnston has shown how to include another fluid of the same mass into a Schr\"odinger code. The two species have separate wavefunctions with separate Schr\"odinger equations, but share a common gravitational potential sourced by the total density of all species: $\nabla^2 V = 4\pi G \sum_i |\psi_i|^2$. In principle this is easy to extend to $N$ species of particles (or fluids). The initial mass of each fluid is allowed to be different, this will be expressed by the fact that the total mass of the wavefunction for each species will be different. It would also be possible to alter the value of $\nu = \hbar/m$ for each species, hence account for different dispersion properties.
...
An alternative method for periodic boundaries would be the use of Fourier transforms, as they are inherently periodic. However, our attempts to implement the kinetic energy operator via an FFT method were unsuccessful: the resulting scheme did not conserve mass to the required precision, likely because the FFT-based kinetic step and the real-space Cayley potential step could not be combined in a way that preserved unitarity at each timestep. A fully spectral approach (both K and V in Fourier space) might avoid this issue but would require a convolution for V, which is computationally expensive.
...
Watanabe (\citeyear{bib_wat}) has shown how to include mesh refinement in a Schr\"odinger solver. Mesh refinement is easily achievable for the kinetic energy operator by use of Watanabe's adhesive operators. The potential energy calculation requires separate consideration and is arguably the harder problem: solving the Poisson equation on a non-uniform mesh is a major obstacle for any AMR implementation. Such an idea is particularly attractive for LSS simulations and should perhaps be one of the first avenues for future wave-mechanics research. It can provide increased spatial resolution in the areas that need it most.
...
The lack of a clear parallelization scheme is a significant obstacle for scaling wave-mechanics to competitive resolutions. The Goldberg/Cayley solver, while excellent for unitarity and stability, may not be the right choice for very large simulations due to the inherently sequential nature of its recursion relations. Future research may need to explore alternative solvers (e.g.\ spectral or pseudo-spectral methods) that are more amenable to parallelization, even if they require additional effort to ensure unitarity.
