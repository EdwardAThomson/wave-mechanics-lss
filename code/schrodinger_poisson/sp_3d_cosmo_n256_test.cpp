...
double NU = 1e-4;                       // well-resolved at N=256 (14% of Nyquist)
...
// ================================================================
// ================================================================
// Spectral kinetic half-step (replaces Goldberg finite-difference)
//
// Applies the exact free-particle propagator in Fourier space:
//   psi_hat(k) *= exp(-i * |k|^2 * dt / (2 * C(a)))
//
// Advantages over Goldberg/Cayley:
// - Exact dispersion relation at all k (no finite-difference error)
// - Naturally periodic (FFT assumes periodic data)
// - No auxiliary function wrapping needed
// - No splitting into x/y/z sweeps needed (3D FFT handles all at once)
//
// The half-step dt/2 is already encoded: dt here is d(ln a)/2.
// ================================================================
void kinetic_half_step_3d(std::vector<Complex>& psi,
                          double C_a, double dt_half,
                          fftw_complex* kin_in, fftw_complex* kin_out,
                          fftw_plan& kin_fwd, fftw_plan& kin_bwd) {
    // Copy psi into FFTW input
    for (int i = 0; i < N3; i++) {
        kin_in[i][0] = psi[i].real();
        kin_in[i][1] = psi[i].imag();
    }

    // Forward FFT
    fftw_execute(kin_fwd);

    // Apply exact propagator: exp(-i * k^2 * dt_half / C_a)
    // where k^2 = kx^2 + ky^2 + kz^2 in box units (k = 2*pi*n)
    for (int ix = 0; ix < N; ix++) {
        double kx = 2.0 * PI * ((ix <= N / 2) ? ix : ix - N);
        for (int iy = 0; iy < N; iy++) {
            double ky = 2.0 * PI * ((iy <= N / 2) ? iy : iy - N);
            for (int iz = 0; iz < N; iz++) {
                double kz = 2.0 * PI * ((iz <= N / 2) ? iz : iz - N);
                double k2 = kx * kx + ky * ky + kz * kz;
                int i = idx(ix, iy, iz);

                // Phase: -k^2 * dt_half / C_a
                double phase = -k2 * dt_half / C_a;
                double c = cos(phase), s = sin(phase);

                // Multiply psi_hat by exp(i*phase)
                double re = kin_out[i][0];
                double im = kin_out[i][1];
                kin_out[i][0] = re * c - im * s;
                kin_out[i][1] = re * s + im * c;
            }
        }
    }

    // Backward FFT
    fftw_execute(kin_bwd);

    // Copy back with FFTW normalisation
    double norm = 1.0 / static_cast<double>(N3);
    for (int i = 0; i < N3; i++) {
        psi[i] = Complex(kin_in[i][0], kin_in[i][1]) * norm;
    }
}
...
// ================================================================
// Read psi from binary checkpoint
// ================================================================
bool read_psi_binary(const std::string& filename,
                     std::vector<Complex>& psi,
                     int& step_out, double& a_out) {
    std::ifstream file(filename, std::ios::binary);
    if (!file.is_open()) return false;
    int n;
    double nu_file;
    file.read(reinterpret_cast<char*>(&step_out), sizeof(int));
    file.read(reinterpret_cast<char*>(&n), sizeof(int));
    file.read(reinterpret_cast<char*>(&a_out), sizeof(double));
    file.read(reinterpret_cast<char*>(&nu_file), sizeof(double));
    if (n != N) { std::cerr << "N mismatch in checkpoint!\n"; return false; }
    psi.resize(N3);
    file.read(reinterpret_cast<char*>(psi.data()), N3 * sizeof(Complex));
    file.close();
    return true;
}
...
//
// Usage:
//   ./sp_3d_cosmo_n256 [nu]              Full run (400 steps)
//   ./sp_3d_cosmo_n256 [nu] chunk 50     Run 50 steps, save checkpoint
//   ./sp_3d_cosmo_n256 [nu] resume 50    Resume from checkpoint, run 50 more
// ================================================================
int main(int argc, char* argv[]) {
    ...
    // Chunk/resume mode
    bool resuming = false;
    int CHUNK_STEPS = 400;  // default: full run
    if (argc >= 3) {
        std::string mode = argv[2];
        if (mode == "chunk" || mode == "resume") {
            CHUNK_STEPS = (argc >= 4) ? std::stoi(argv[3]) : 50;
    ...
    std::cout << "d(ln a) = " << d_lna << ", N_steps = " << TOTAL_STEPS << "\n\n";
    ...
    // --- FFTW setup for spectral kinetic step ---
    // Separate workspace so kinetic and Poisson don't interfere
    fftw_complex* kin_in = fftw_alloc_complex(N3);
    fftw_complex* kin_out = fftw_alloc_complex(N3);
    fftw_plan kin_fwd = fftw_plan_dft_3d(N, N, N, kin_in, kin_out,
                                          FFTW_FORWARD, FFTW_ESTIMATE);
    fftw_plan kin_bwd = fftw_plan_dft_3d(N, N, N, kin_out, kin_in,
                                          FFTW_BACKWARD, FFTW_ESTIMATE);
    ...
    // --- Output setup ---
    std::string outdir = "output/sp_cosmo_n256_nu_" + std::to_string(NU);
    system(("mkdir -p " + outdir).c_str());
    std::string checkpoint_file = outdir + "/checkpoint.bin";
    ...
    if (resuming) {
        int ck_step; double ck_a;
        if (read_psi_binary(checkpoint_file, psi, ck_step, ck_a)) {
            start_step = ck_step;
            a = ck_a;
            // Recompute mass
            mass0 = 0.0;
            for (int i = 0; i < N3; i++) mass0 += std::norm(psi[i]);
            mass0 *= dV;
            std::cout << "RESUMED from checkpoint: step=" << start_step
                      << " a=" << a << " z=" << 1.0/a - 1.0
                      << " mass=" << mass0 << "\n\n";
        } else {
            std::cerr << "ERROR: no checkpoint found at " << checkpoint_file << "\n";
            return 1;
        }
    } else {
    ...
    int end_step = std::min(start_step + CHUNK_STEPS, TOTAL_STEPS);

    printf("Running steps %d -> %d (of %d total)\n\n", start_step + 1, end_step, TOTAL_STEPS);
    ...
    printf("%-6d  %-7.4f  %-6.2f  %-18.15f  %+.2e  %-8.1f  %-7.1f  -\n",
           start_step, a, 1.0/a - 1.0, mass0, 0.0, g_eff_of_a(a), lambda_of_a(a, d_lna));
    ...
    for (int step = start_step + 1; step <= end_step; step++) {
        ...
        double dt_half = d_lna / 2.0;  // half-step for Strang splitting

        kinetic_half_step_3d(psi, C_a, dt_half, kin_in, kin_out, kin_fwd, kin_bwd);
    ...
    int steps_run = end_step - start_step;
    printf("\nTotal: %.1f s (%.3f s/step)\n", total, total / std::max(steps_run, 1));

    // Save checkpoint for resume
    write_psi_binary(checkpoint_file, psi, end_step, a);
    printf("Checkpoint saved: step=%d, a=%.6f, z=%.2f\n", end_step, a, 1.0/a - 1.0);
    if (end_step < TOTAL_STEPS) {
        printf("Resume with: ./sp_3d_cosmo_n256 %g resume %d\n", NU, CHUNK_STEPS);
    } else {
    ...
    fftw_free(work_in);
    fftw_free(work_out);
    fftw_destroy_plan(kin_fwd);
    fftw_destroy_plan(kin_bwd);
    fftw_free(kin_in);
    fftw_free(kin_out);
