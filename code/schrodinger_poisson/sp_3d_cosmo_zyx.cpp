...
// ================================================================
// 3D kinetic half-step: sweep along z, y, x (REVERSED from default)
// Test: does the bulk drift direction follow the sweep order?
// ================================================================
void kinetic_half_step_3d(std::vector<Complex>& psi, double lambda) {
    Complex pencil[N];

    // z-sweep FIRST (was last in default)
    for (int ix = 0; ix < N; ix++)
        for (int iy = 0; iy < N; iy++) {
    ...
    // x-sweep LAST (was first in default)
    for (int iy = 0; iy < N; iy++)
        for (int iz = 0; iz < N; iz++) {
...
int main(int argc, char* argv[]) {
    ...
    // --- Output setup ---
    std::string outdir = "output/sp_cosmo_zyx_nu_" + std::to_string(NU);
