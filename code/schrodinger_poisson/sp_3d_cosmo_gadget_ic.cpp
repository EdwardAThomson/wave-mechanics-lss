...
double NU = 3e-4;                       // smallest practical nu for N=128
...
int main(int argc, char* argv[]) {
    ...
    // --- Read ICs from GADGET-extracted binary files ---
    // Generated by extract_gadget_ics.py (TSC deposit + Poisson-derived phi_v)
    std::string ic_dir = "output/gadget_ics";
    std::cout << "Reading GADGET ICs from " << ic_dir << "/...\n";

    auto read_field = [&](const std::string& filename, std::vector<double>& field) {
        ...
        f.read(reinterpret_cast<char*>(&a_file), sizeof(double));
        if (n_file != N) {
            std::cerr << "ERROR: N mismatch: file has " << n_file << ", code expects " << N << "\n";
            exit(1);
        }
        field.resize(N3);
        f.read(reinterpret_cast<char*>(field.data()), N3 * sizeof(double));
        std::cout << "  " << filename << ": N=" << n_file << ", a=" << a_file << "\n";
    };

    std::vector<double> delta_init(N3);
    std::vector<double> phi_v(N3);
    read_field(ic_dir + "/delta_gadget.bin", delta_init);
    read_field(ic_dir + "/phi_v_gadget.bin", phi_v);
    ...
    double phi_v_max = 0.0;
    for (int i = 0; i < N3; i++) {
        if (std::abs(phi_v[i]) > phi_v_max) phi_v_max = std::abs(phi_v[i]);
    }
    double k_nyq = PI * N;
    // s_max ≈ max|grad(phi_v)| ≈ phi_v_max * k_typical (rough estimate)
    // More precise: read s_max from the extraction script output
    std::cout << "phi_v max = " << phi_v_max << "\n";
    std::cout << "Phase max = phi_v_max/nu = " << phi_v_max / NU << " radians\n";
    ...
    // --- Output setup ---
    std::string outdir = "output/sp_cosmo_gadget_ic_nu_" + std::to_string(NU);
