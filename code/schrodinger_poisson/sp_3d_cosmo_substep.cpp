...
// ================================================================
// Potential step with sub-stepping
//
// The Cayley approximation of exp(-i*x) breaks down for |x| >> 1.
// With large L0 (classical regime), g_eff*V*dt can reach ~6-14,
// causing the phase to scramble rather than accumulate coherently.
//
// Fix: split the single Cayley rotation into N_SUB smaller rotations,
// each with phase argument N_SUB times smaller. The potential V is
// held fixed during sub-stepping (it only changes at the outer
// Strang-splitting level), so this is cheap — just N_SUB pointwise
// complex multiplications per grid point.
// ================================================================
constexpr int N_SUB = 100;  // sub-steps per potential step

void potential_step_3d(std::vector<Complex>& psi,
                       const std::vector<double>& V,
                       double g_eff, double dt) {
    double dt_sub = dt / N_SUB;
    for (int sub = 0; sub < N_SUB; sub++) {
        for (int i = 0; i < N3; i++) {
            double vdt2 = g_eff * V[i] * dt_sub / 2.0;
