...
int main(int argc, char* argv[]) {
    ...
    // --- Construct wavefunction: density only, NO velocity phase ---
    // psi = sqrt(1 + delta), real-valued, zero initial velocity.
    // Avoids Madelung phase aliasing entirely.
    // Gravity will self-consistently develop the velocity field.
    std::cout << "\nConstructing zero-velocity wavefunction (density only)...\n";
    std::vector<Complex> psi(N3);
    for (int i = 0; i < N3; i++) {
        double amp = sqrt(std::max(1.0 + delta_init[i], 0.0));
        psi[i] = Complex(amp, 0.0);
    }
    ...
    // --- Output setup ---
    std::string outdir = "output/sp_cosmo_nophase_nu_" + std::to_string(NU);
