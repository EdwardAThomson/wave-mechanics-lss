...
int main(int argc, char* argv[]) {
    ...
    // --- Output setup ---
    std::string outdir = "output/sp_cosmo_static_nu_" + std::to_string(NU);
    ...
    // --- Time loop (STATIC: no expansion, fixed coefficients at a_init) ---
    double lam_fixed = lambda_of_a(a_init, d_lna);
    double g_eff_fixed = g_eff_of_a(a_init);
    std::cout << "STATIC MODE: lambda = " << lam_fixed
              << ", g_eff = " << g_eff_fixed << " (fixed)\n\n";

    for (int step = 1; step <= N_STEPS; step++) {
        kinetic_half_step_3d(psi, lam_fixed);
        poisson_solve_3d(psi, V, work_in, work_out, plan_fwd, plan_bwd);
        potential_step_3d(psi, V, g_eff_fixed, d_lna);
        kinetic_half_step_3d(psi, lam_fixed);

        // a does NOT advance — static universe
