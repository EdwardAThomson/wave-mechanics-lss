...
int main(int argc, char* argv[]) {
    ...
    // --- Remove bulk flow from ICs ---
    // Compute mean probability current, derive bulk velocity, apply
    // Galilean boost to set <j> = 0. Applied ONCE to the ICs only.
    {
        double inv_2eps = 1.0 / (2.0 * epsilon);
        double mean_jx = 0.0, mean_jy = 0.0, mean_jz = 0.0, mean_rho = 0.0;

        for (int ix = 0; ix < N; ix++) {
            int ixp = (ix + 1) % N, ixm = (ix - 1 + N) % N;
            for (int iy = 0; iy < N; iy++) {
                int iyp = (iy + 1) % N, iym = (iy - 1 + N) % N;
                for (int iz = 0; iz < N; iz++) {
                    int izp = (iz + 1) % N, izm = (iz - 1 + N) % N;
                    int i = idx(ix, iy, iz);
                    Complex ps = std::conj(psi[i]);
                    mean_jx += (ps * (psi[idx(ixp,iy,iz)] - psi[idx(ixm,iy,iz)])).imag();
                    mean_jy += (ps * (psi[idx(ix,iyp,iz)] - psi[idx(ix,iym,iz)])).imag();
                    mean_jz += (ps * (psi[idx(ix,iy,izp)] - psi[idx(ix,iy,izm)])).imag();
                    mean_rho += std::norm(psi[i]);
                }
            }
        }
        // v_bulk = nu * <Im(psi* dpsi/dx)> / <rho>  (the inv_2eps cancels in v/nu)
        double vx_bulk = NU * inv_2eps * mean_jx / mean_rho;
        double vy_bulk = NU * inv_2eps * mean_jy / mean_rho;
        double vz_bulk = NU * inv_2eps * mean_jz / mean_rho;

        std::cout << "IC bulk velocity: (" << vx_bulk << ", " << vy_bulk
                  << ", " << vz_bulk << ")\n";

        // Galilean boost: psi *= exp(-i * v_bulk . x / nu)
        for (int ix = 0; ix < N; ix++) {
            double x = (ix + 0.5) * epsilon;
            for (int iy = 0; iy < N; iy++) {
                double y = (iy + 0.5) * epsilon;
        ...
        std::cout << "Bulk flow removed from ICs (Galilean boost)\n";
    }
    ...
    // --- Output setup ---
    std::string outdir = "output/sp_cosmo_icfix_nu_" + std::to_string(NU);
