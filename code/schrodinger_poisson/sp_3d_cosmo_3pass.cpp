...
void goldberg_sweep(Complex* pencil, double lambda) {
    ...
    // e: THREE passes (testing if 2 is sufficient)
    e[0] = alpha;
    for (int j = 1; j < N; j++) e[j] = alpha - 1.0 / e[j - 1];
    e[0] = alpha - 1.0 / e[N - 1];
    for (int j = 1; j < N; j++) e[j] = alpha - 1.0 / e[j - 1];
    e[0] = alpha - 1.0 / e[N - 1];  // 3rd pass
    for (int j = 1; j < N; j++) e[j] = alpha - 1.0 / e[j - 1];
    ...
    for (int j = 1; j < N; j++) f[j] = Omega[j] + f[j - 1] / e[j - 1];
    f[0] = Omega[0] + f[N - 1] / e[N - 1];  // 3rd pass
    for (int j = 1; j < N; j++) f[j] = Omega[j] + f[j - 1] / e[j - 1];

    // psi: THREE backward passes
    pencil[N - 1] = (pencil[0] - f[N - 1]) / e[N - 1];
    for (int j = N - 2; j >= 0; j--) pencil[j] = (pencil[j + 1] - f[j]) / e[j];
    pencil[N - 1] = (pencil[0] - f[N - 1]) / e[N - 1];
    for (int j = N - 2; j >= 0; j--) pencil[j] = (pencil[j + 1] - f[j]) / e[j];
    pencil[N - 1] = (pencil[0] - f[N - 1]) / e[N - 1];  // 3rd pass
    for (int j = N - 2; j >= 0; j--) pencil[j] = (pencil[j + 1] - f[j]) / e[j];
}
...
int main(int argc, char* argv[]) {
    ...
    // --- Output setup ---
    std::string outdir = "output/sp_cosmo_3pass_nu_" + std::to_string(NU);
